...
5. Qual é a diferença entre tags like {bold} e style.fg?
```
...
**Problem:** Você adiciona componentes mas não vê nada na tela.
**Solution:** Sempre chame screen.render() após modificar componentes.

### Pitfall 2: Memory Leaks com Timers
**Problem:** setInterval nunca é limpo, causando memory leaks.
**Solution:** Sempre guarde referências a timers e limpe no cleanup.

### Pitfall 3: Layout Fixo vs Responsivo
**Problem:** Usar posições fixas (top: 10) quebram em terminais pequenos.
**Solution:** Use posições relativas (top: "30%") quando possível.
...
**Solution:** Teste em múltiplos terminais, use cores básicas (16 cores).
```
...
**Symptoms:** Erro ao executar bun --bun run dev
...
**Symptoms:** Linhas não se alinham, texto sobreposto
**Diagnosis:** Terminal muito pequeno ou fonte não monoespaçada
**Solution:**
- Aumente o tamanho do terminal (mínimo 80x24)
- Use fonte monoespaçada (Courier, Consolas, etc.)
...
**Solution:**
- Verifique se screen.key() está correto
- Use o nome da tecla correto (ex: "C-c" não "ctrl-c")
...
- Teste conexão com wscat ou ferramenta similar

### Issue: Dados não atualizam
**Symptoms:** Valores ficam estáticos mesmo com WebSocket conectado
**Diagnosis:** Event handler não registrado ou assetId incorreto
**Solution:**
- Verifique se onUpdate está sendo chamado
- Confirme assetId corresponde ao token esperado

### Issue: Erro "content is not a function"
**Symptoms:** Erro ao tentar atualizar conteúdo
**Diagnosis:** Tentando usar .content() em vez de .setContent()
**Solution:** Use setContent(string) não content()
```
...
|----------|---------|----------|-----|-----|----------|
| Facilidade de SSH | ❌ Difícil | ❌ Impossível | ✅ Fácil | ✅ Fácil | TUI |
| Performance | ⚠️ Browser overhead | ❌ Pesado | ✅ Leve | ✅ Leve | TUI |
| Distribuição | ⚠️ Servidor required | ⚠️ Binário grande | ✅ Single binary | ✅ Single binary | TUI |
| Visual rico | ✅ HTML/CSS | ✅ HTML/CSS | ❌ Sem visual | ⚠️ Limitado | Web |
| Aparência "hacker" | ❌ Corporate | ❌ App comum | ⚠️ Simples | ✅ Premium | TUI |
| Recursos de sistema | ⚠️ Limitado | ✅ Acesso total | ✅ Acesso total | ✅ Acesso total | Empate |
...
- Web App: Para usuários não-técnicos que preferem interface visual
- Electron: Se precisasse de recursos gráficos avançados
...
**Por que Blessed:**
- ✅ Maduro: Estável há anos, amplamente usado
- ✅ Simples: API direta sem camadas de abstração
- ✅ Completo: Suporta layouts, mouse, forms
- ✅ Documentação: Muitos exemplos e tutoriais
- ✅ Compatibilidade: Funciona em Node.js e Bun
...
- ❌ Espaço: Requer terminal maior (mínimo 80x24 recomendado 120x40)
- ❌ Aprendizado: Mais informações para processar
...
- ✅ WebSocket: Já recebemos updates em tempo real

**Estratégia de atualização:**
- Radar: 60 segundos (muda pouco)
- Orderbook: 3 segundos (muda muito)
- History: 30 segundos (dados históricos)
...
```
**Why it's bad:** Você nunca sabe que algo falhou.
**Solution:**
```typescript
try {
  await riskyOperation();
} catch (err) {
  logger.error("Operação falhou", { error: err });
...
**Problem:** Múltiplos clientes sincronizam retries.
**Solution:** Adicione jitter aleatório ao backoff.
...
**Solution:** Logar endpoint, parâmetros, tentativas, etc.

### Pitfall 7: Exponential Backoff Errado
**Problem:** Cresce muito rápido (2^10 = 1024x)
**Solution:** Cap em tempo máximo e limite de tentativas.
```
...
**Alternativas:**
1. Jest - Test runner padrão do ecossistema Node.js
2. Vitest - Test runner rápido compatível com Jest
3. Bun Test - Test runner embutido no Bun ✅ **ESCOLHIDO**

**Comparação:**

| Critério | Jest | Vitest | Bun Test |
|----------|------|--------|----------|
| Velocidade | Lento | Rápido | Mais rápido |
| Setup | npm install | npm install | Embutido |
| Compatibilidade | Máxima | Alta | Boa |
| Watch Mode | Sim | Sim | Sim |
| Coverage | Sim | Sim | Sim |
| Mocking | Avançado | Avançado | Básico |
| Tamanho | Grande | Médio | Zero |

**Por que Bun Test:**
- ✅ Performance: 100x mais rápido que Jest
- ✅ Zero setup: Já vem com Bun
- �Compatibilidade: Projeto já usa Bun
...
3. Híbrido - Unit com mocks, integração com reais ✅ **ESCOLHIDO**

**Estratégia:**
- Unit tests: 100% mocks, rápidos, isolados
- Integração: APIs reais quando possível, senão mocks gravados
- E2E: Sempre APIs reais

### Decisão 3: Coverage target: 80%, 90%, ou 100%?

**Metas:**
- 100% coverage ❌ Impraticável
- 90% coverage ⚠️ Custo/benefício questionável
- 80% coverage ✅ **ESCOLHIDO** - Bom equilíbrio

**Racional:**
- Código de UI é difícil de testar (TUI)
- Error handling edge cases têm baixo ROI
- Type safety já previne muitos bugs
- Foco em código crítico de negócio

### Decisão 4: TDD ou Test-After?

**Abordagens:**
1. TDD - Escreve testes antes do código
2. Test-After - Escreve testes depois do código
3. Test-During - Escreve testes durante desenvolvimento ✅ **REALIDADE**

**O que fazemos:**
- Testes para código novo: TDD idealmente
- Testes para código existente: Test-after
...
1. Trump vencerá as eleições de 2024?
2. Elon Musk postará no X antes de 2025?
3. BTC estará acima de $100k em 2025?
4. Ethereum superará $5k em 2025?
5. Argentina vencerá a Copa do Mundo?
```

**Verificação:**
- ✅ Viu 5 mercados listados?
- ✅ As perguntas estão em português?
- ✅ Não houve erros?

**Erros comuns:**
- `Error: Cannot find module './src/api'` - Você está no diretório errado
- `fetch failed` - Sem conexão com internet ou API fora do ar
