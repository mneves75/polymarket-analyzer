...
class MarketService {
	...
	async getRadar(
		limit: number,
	): Promise<Result<MarketInfo[], NetworkError | DataError>> {
		const now = Date.now();
		...
		if (
			...
			logger.debug("Radar cache hit", {
				cacheAge: now - this.radarCache.timestamp,
			});
		...
		try {
			...
			if (!Array.isArray(markets)) {
				return err(
					new DataError("Invalid API response: not an array", {
						type: typeof markets,
					}),
				);
			}
			...
			return err(
				new NetworkError(`Failed to load radar: ${message}`, { limit }),
			);
		}
	}
