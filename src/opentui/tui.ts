...
function computeHealthScore(state: DashboardState): HealthScore {
	...
	const bidDepth =
		state.orderbook?.bids?.reduce((sum, l) => sum + l.size, 0) ?? 0;
	const askDepth =
		state.orderbook?.asks?.reduce((sum, l) => sum + l.size, 0) ?? 0;
...
function updateRadarPanel(
	...
	try {
		...
		for (let i = 0; i < displayMarkets.length; i++) {
			...
			const outcomeName =
				market.outcomes?.[state.outcomeIndex]?.substring(0, 12) || "-";
		...
		logger.debug("updateRadarPanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateMarketPanel(
	...
	try {
		...
		const tokenId =
			focusMarket.clobTokenIds[state.outcomeIndex] ??
			focusMarket.clobTokenIds[0];
		const outcome =
			focusMarket.outcomes[state.outcomeIndex] ||
			`OUTCOME_${state.outcomeIndex + 1}`;
		...
		logger.debug("updateMarketPanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updatePulsePanel(
	...
	try {
		...
		pulseBox.add(
			new TextRenderable(renderer, { content: t`${bold("Pulse")}` }),
		);
		...
		logger.debug("updatePulsePanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateOrderbookPanel(
	...
	try {
		...
		orderbookBox.add(
			new TextRenderable(renderer, { content: t`${bold("Orderbook")}` }),
		);
		...
		logger.debug("updateOrderbookPanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateHistoryPanel(
	...
	try {
		...
		historyBox.add(
			new TextRenderable(renderer, { content: t`${bold("History")}` }),
		);
		...
		logger.debug("updateHistoryPanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateAlertsPanel(
	...
	try {
		...
		alertsBox.add(
			new TextRenderable(renderer, { content: t`${bold("Alerts & Status")}` }),
		);
		...
		logger.debug("updateAlertsPanel failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateDetailModal(
	...
	try {
		...
		const content = generateDetailContent({
			...
			// OpenTUI doesn't have granular loading state yet - use reasonable defaults
			loading: {
				pricing: state.bestBid === undefined && state.bestAsk === undefined,
				orderbook: state.orderbook === null && !state.noOrderbook,
				history: state.historySeries.length === 0,
			},
		});
		...
		logger.debug("updateDetailModal failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function updateHelpModal(
	...
	try {
		...
		logger.debug("updateHelpModal failed", {
			error: error instanceof Error ? error.message : String(error),
		});
	}
}
...
function renderAll(panels: Panels, renderer: CliRenderer): void {
	...
	if (state.showDetail) {
		...
		if (
			!panels.mainContainer.getChildren().find((c) => c.id === "detail-modal")
		) {
	...
	if (state.showHelp) {
		...
		if (
			!panels.mainContainer.getChildren().find((c) => c.id === "help-modal")
		) {
		...
		try {
			...
			switch (key.name) {
				...
				case "a": {
					...
					stateManager.setLastAlert(
						skipState ? "Auto-skip enabled" : "Auto-skip disabled",
					);
					renderAll(panels, renderer);
					break;
				}
