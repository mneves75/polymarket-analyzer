...
export function connectMarketWs(assetIds: string[], handlers: WsHandlers) {
  ...
  const connect = () => {
    ...
    ws.addEventListener("message", (event) => {
      ...
      try {
        ...
        parsed.books.forEach((book) => handlers.onBook?.(book.assetId, book.book));
        parsed.updates.forEach((update) => handlers.onUpdate(update));
      } catch (err) {
  ...
  const sendOperation = (ids: string[], operation: "subscribe" | "unsubscribe") => {
    if (!ws || ws.readyState !== WebSocket.OPEN) return;
    const payload = {
      assets_ids: ids,
      operation,
