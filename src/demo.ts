...
import { loadRadar, type ResolveOptions, resolveMarket } from "./market";
...
export async function runSnapshot(opts: DemoOptions) {
	...
	let orderbook: Record<string, unknown> | null | undefined;
	let prices:
		| { buy: Record<string, unknown>; sell: Record<string, unknown> }
		| null
		| undefined;
	let midpoint: Record<string, unknown> | null | undefined;
	let history: Record<string, unknown> | null | undefined;
	let holders: Array<Record<string, unknown>> | null | undefined;
	...
	const [orderbookRes, pricesRes, midpointRes, historyRes, holdersRes] =
...
export async function listMarkets(limit = CONFIG.radarLimit, json = false) {
	...
	radar.forEach((market, idx) => {
		...
		console.log(
			`${String(idx + 1).padStart(2, "0")} | ${name} | ${outcome} | ${market.conditionId}`,
		);
	});
}
