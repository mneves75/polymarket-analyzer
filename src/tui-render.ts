import { THEME, type HealthScore } from "./tui-types";
...
export function computeHealthScore(opts: {
  ...
  let score = 0;
  const spread = opts.bestBid !== undefined && opts.bestAsk !== undefined
    ? opts.bestAsk - opts.bestBid
    : undefined;
  const bidDepth = opts.orderbook?.bids?.reduce((sum, l) => sum + l.size, 0) ?? 0;
  const askDepth = opts.orderbook?.asks?.reduce((sum, l) => sum + l.size, 0) ?? 0;
