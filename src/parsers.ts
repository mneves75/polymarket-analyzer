 ...
 */
export function extractMidpoint(response: MidpointResponse): number | undefined {
 ...
 */
export function normalizeOrderbook(response: OrderbookResponse): OrderbookState {
  ...
  const negRisk = response['neg_risk'] === true ? true : response['neg_risk'] === false ? false : undefined;
  ...
  return levels
    .map((level): OrderbookLevel | null => {
      ...
      if (level && typeof level === 'object') {
        ...
        const size = asNumber(record['size'] ?? record['s'] ?? record['amount'] ?? record['quantity']);
        if (price !== undefined && size !== undefined) {
          return { price, size };
        }
        return null;
 ...
 */
export function normalizeHolders(data: HoldersResponse | Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  ...
  if (Array.isArray(data)) {
    ...
    const first = data[0];
    if (!first) return [];
    holdersArray = (first['holders'] as Array<Record<string, unknown>> | undefined) ?? [];
  } else {
    holdersArray = (data['holders'] as Array<Record<string, unknown>> | undefined) ?? [];
  }
