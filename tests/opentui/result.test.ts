...
describe("Result type", () => {
  ...
  describe("err()", () => {
    ...
    it("works with string errors", () => {
      ...
      expect(result.ok).toBe(false);
      expect((result as { ok: false; error: string }).error).toBe("Something went wrong");
    });
  });
  ...
  describe("mapErr()", () => {
    it("maps error values", () => {
      ...
      expect(mapped.ok).toBe(false);
      expect((mapped as { ok: false; error: Error }).error.message).toBe("original error");
    });
  ...
  describe("fromPromise()", () => {
    ...
    it("returns err for rejected promises with Error", async () => {
      ...
      expect(result.ok).toBe(false);
      expect((result as { ok: false; error: Error }).error.message).toBe("Test error");
    });
    ...
    it("wraps non-Error rejections in Error", async () => {
      ...
      expect((result as { ok: false; error: Error }).error.message).toBe("string error");
    });
    ...
    it("handles async functions", async () => {
      ...
      expect(result.ok).toBe(true);
      expect((result as { ok: true; value: string }).value).toBe("async result");
    });
  });
});
