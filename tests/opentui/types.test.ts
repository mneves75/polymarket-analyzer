...
describe("validateDashboardOptions", () => {
	...
	it("throws on invalid limit", () => {
		expect(() => validateDashboardOptions({ limit: 0 })).toThrow(
			"Invalid limit",
		);
		expect(() => validateDashboardOptions({ limit: -1 })).toThrow(
			"Invalid limit",
		);
		expect(() => validateDashboardOptions({ limit: 1001 })).toThrow(
			"Invalid limit",
		);
	});

	it("throws on invalid intervalMs", () => {
		expect(() => validateDashboardOptions({ intervalMs: 50 })).toThrow(
			"Invalid intervalMs",
		);
		expect(() => validateDashboardOptions({ intervalMs: 100000 })).toThrow(
			"Invalid intervalMs",
		);
	});
