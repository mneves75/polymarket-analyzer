...
describe("StateManager", () => {
  ...
  describe("radar management", () => {
    ...
    it("setRadar throws on invalid input", () => {
      expect(() => stateManager.setRadar(null as unknown as MarketInfo[])).toThrow(DataError);
      expect(() => stateManager.setRadar("invalid" as unknown as MarketInfo[])).toThrow(DataError);
    });
  ...
  describe("pricing state", () => {
    ...
    it("setBestAsk updates ask", () => {
      stateManager.setBestAsk(0.60);
      expect(stateManager.get().bestAsk).toBe(0.60);
    });
