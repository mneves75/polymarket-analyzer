...
describe("http", () => {
			...
			it("parses JSON error response body", async () => {
				mockFetch.mockImplementation(() =>
					Promise.resolve({
						...
						text: () =>
							Promise.resolve(JSON.stringify({ error: "Invalid parameter" })),
						get headers() {
							return new Headers();
						},
					} as Response),
	...
	describe("withQuery", () => {
		it("adds query parameters to URL", () => {
			...
			expect(result).toBe(
				"https://api.test.com/search?q=polymarket&limit=10&active=true",
			);
		});
		...
		it("preserves existing query parameters", () => {
			...
			expect(result).toBe(
				"https://api.test.com/search?existing=param&new=value",
			);
		});
		...
		it("handles numbers and booleans", () => {
			...
			expect(result).toBe(
				"https://api.test.com/data?count=42&enabled=false&name=test",
			);
		});
