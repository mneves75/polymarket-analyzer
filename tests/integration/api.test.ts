...
describe("API Integration Tests", () => {
  ...
  describe("fetchMarketByConditionId", () => {
    ...
    it("returns null for invalid condition ID", async () => {
      const market = await fetchMarketByConditionId("0xinvalidconditionid1234567890123456789012345678901234567890123456789012345678");
  ...
  describe("getPrices", () => {
    it("fetches current prices for token IDs", async () => => {
      ...
      const markets = await fetchMarkets({ limit: 5 });
      const marketWithToken = markets.find((m) => m.clobTokenIds && m.clobTokenIds.length > 0);
  ...
  describe("getOrderbook", () => {
    ...
    it("handles invalid token ID gracefully", async () => {
      ...
      const orderbook = await getOrderbook("0xinvalidtokenid1234567890123456789012345678901234567890123456789012345678");
  ...
  describe("rate limiting", () => {
    it("respects rate limits when making multiple requests", async () => {
      const requests = Array.from({ length: 5 }, () => fetchMarkets({ limit: 1 }));
